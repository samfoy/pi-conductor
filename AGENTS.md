...
Versioning lives in the PRD's "Implementation phases" section. **Current state: v0.12 shipped** — all milestones v0.1–v0.10 plus v0.12 (steering) are live. Highlights through v0.10: read-only scaffold, background + foreground spawn with inline-streamed transcript, queue, ensemble panel, conductor mode prompt, focused-stream overlay, ensemble_send / pause / resume / kill / focus, filtered context inheritance (incl. v0.12's `filtered_compact`), Esc-to-detach, /conductor history, strict-overseer mode with default-off, inherit_skills frontmatter, run-record GC capstone with auto + manual + reconcile-orphans + cold-archive + delete + user-pinning, sub-agent watchdog with soft/hard thresholds + per-spawn kill_on_stall + UX surfaces. v0.12 adds: per-spawn `steerable: true` (default OFF) to launch sub-agents in pi RPC mode, `streaming_behavior: "auto"|"steer"|"follow_up"|"resume"` on `ensemble_send`, `/conductor send` slash command, single-writer RPC stdin queue, `lastEventAt` bump asymmetry between RPC `response` and host-blocking `extension_ui_request`, and an orphaned-RPC reconcile branch on conductor restart. Next up (per PRD v0.10+ planned list): worktree-per-persona, on_complete_hook quality gates (v0.11, in flight); smaller items live in `docs/backlog.md`.
