...
- 2026-05-29 — **v0.11 on_complete_hook quality gates — R8 + R9 soft-compat callouts.** R8: external consumers of `record.json` will see `"status": "hook_failed"` as a new terminal value; treat it as failure-class terminal (not "completed"). R9: `final.md` mtime ordering contract: in v0.11, `final.md` is written BEFORE the hook fires; tooling that assumes `final.md` mtime ≈ run end time must account for the hook’s additional wall time. Neither is a breaking API change (see `docs/v0.11-on-complete-hook-design.md` R8/R9). SHAs and test delta filled by finalizer.
