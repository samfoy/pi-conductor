...
- 2026-05-19 — **v0.9 run-record GC capstone — locked decisions D1–D8** (8 slice commits `4f68833..fa5aff0`; design `docs/v0.9-gc-design.md`).
  ...
  - Also closed: D4 sidecar `.pinned` file (slice 4, `9060247`); D5 stale-`running` reconcile to `killed` with marker, never delete in same pass (slice 2, `9b2bc1c`); D6 dual trigger — auto on `session_start` (debounced) + manual `/conductor gc` (slices 5–6, `def2256` / `e0fb122`); D7 cold-archive preserves `session/`, full-delete breaks resume by design. Doctor surface (slice 7, `d77afbf`) and history annotations (slice 8, `fa5aff0`). Test count: 744 → 874 (+130) across the GC capstone.
