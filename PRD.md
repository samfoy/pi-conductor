...
- 2026-05-20 — **v0.10 sub-agent watchdog — locked decisions Q1–Q5** (4 slice commits `c8623cb..4734298`; design `docs/v0.10-watchdog-design.md`).
  ...
  - **Q3 — Soft advisory triggers a turn (`triggerTurn: true`).** Surfaces the `<sub-agent-stalled>` envelope during normal flow rather than waiting for the conductor's next action. Mirrors completion-envelope shape (`pushCompletionNotification`) so the parent's reaction loop is uniform across stall-advisory and completion-notification deliveries. Reference §7 Q3.
